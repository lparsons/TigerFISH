function [spb sp img] = locSpots_3( X, Thresh )
...
%     end
    ...
    [sl hat_xyz Rsq] = spot_regress_fun( pix_5, 11 );
    
    

    Mean = sum( pix_3(:) ) * i_num3;
    Background = (sum(pix_5(:)) - sum(pix_4(:)) ) * i_num;
    ...
    if Rsq      > 0.25 || ...
       Contrast > Thresh %1.10 
  
       j=j+1;       
 
       sp.mean(j) =  Mean;      
       sp.background(j) = Background;
       pix = pix_4(:) - Background; %Background_median;
